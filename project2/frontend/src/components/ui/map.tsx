...
> = {
  /** Optional unique identifier prefix for the cluster source/layers */
  id?: string;
  ...
  /** Color for unclustered individual points (default: "#3b82f6") or expression */
  pointColor?: any;
  /** Custom cluster properties for MapLibre clustering aggregation */
  clusterProperties?: any;
  /** Custom color expression for clusters (overrides clusterColors step pattern) */
  clusterColor?: any;
  /** Render clusters as SVG doughnut charts reflecting distribution */
  renderAsPie?: boolean;
...
>({
  ...
  // Handle styleimagemissing event to lazily generate pie charts
  useEffect(() => {
    if (!isLoaded || !map || !renderAsPie) return;

    const handleImageMissing = (e: any) => {
      const id = e.id;
      const match = id.match(/^pie-(\d+)-(\d+)-(\d+)$/);
      if (!match) return;
  ...
  useEffect(() => {
    ...
    if (renderAsPie) {
      // Add symbol layer representing clusters as pie charts with centered text count
      map.addLayer({
        id: clusterLayerId,
        type: "symbol",
        source: sourceId,
        ...
        layout: {
          "icon-image": [
            "concat",
            "pie-",
            ["coalesce", ["get", "western"], 0],
            "-",
            ["coalesce", ["get", "chinese"], 0],
            "-",
            ["coalesce", ["get", "global"], 0]
          ],
          "icon-size": [
            "step",
            ["get", "point_count"],
      ...
      // Add standard cluster circles layer
      map.addLayer({
        id: clusterLayerId,
        type: "circle",
        source: sourceId,
  ...
  useEffect(() => {
    ...
    stylePropsRef.current = { clusterColors, clusterThresholds, pointColor, clusterColor };
  }, [
    isLoaded,
    map,
    clusterLayerId,
  ...
  useEffect(() => {
    ...
    // Cursor style and popup handlers
    const handleMouseEnterCluster = (e: any) => {
      map.getCanvas().style.cursor = "pointer";
      ...
      if (renderAsPie) {
        ...
        const w = Number(f.properties?.western || 0);
        const c = Number(f.properties?.chinese || 0);
        const g = Number(f.properties?.global || 0);
        const total = w + c + g || Number(f.properties?.point_count || 1);
        ...
        const popupContent = `
          <div class="custom-tooltip-content font-mono text-xs p-1" style="min-width: 140px;">
            <div class="font-bold border-b border-border pb-1 mb-1 text-[11px] uppercase tracking-wider text-text-muted">
        ...
        let size = 36;
        if (total >= 100 && total < 750) size = 48;
        else if (total >= 750) size = 60;
