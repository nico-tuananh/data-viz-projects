"""Daily aggregates endpoint (for timeline charts)."""
import math

from fastapi import APIRouter, Query
from backend.data_loader import filter_daily, DATE_MIN, DATE_MAX, MEDIA_GROUPS
...
def get_daily(
    ...
    df = filter_daily(start, end, media_groups, direction)

    if df.empty:
        return {"count": 0, "records": []}

    grouped = (
        df.groupby(["Date", "MediaGroup"])
        .agg(
